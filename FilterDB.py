    ...
    def walker(self, node):
        ...
        for child in node.children:
            if type(child) is bs4.element.NavigableString:
                searchTexts = ['(dine?|ikke|disse|bort|(b|B)rowser(.){0,50}|set|websted| en|til| de|(S|s)ome|will|synkronisere|slette(r|s)?( du)?|lger?|rence|nvend(er|es|te)?|bru(g|k)er?|(u|U)ses?|a(f|v)|vi|(A|a)ccept(erer|er)?|tter|bout|(o|O)m|to|vores|bruges?|elle|of) (c|C)ookies?',
